...
class RouteCourseConfigurator implements RouteConfigurator {
  ...
  public void configureRoutes(Vertx vertx, Router router, JsonObject config) {
      ...
      DeliveryOptions options =
          DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
              ...
              .addHeader(MessageConstants.MSG_HEADER_OP,
                  MessageConstants.MSG_OP_COURSE_COLLABORATOR_UPDATE)
    ...
    router.put(RouteConstants.EP_COURSE_CONTENT_REORDER).handler(routingContext -> {
      ...
      DeliveryOptions options =
          DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
              .setSendTimeout(mbusTimeout * 1000)
              .addHeader(MessageConstants.MSG_HEADER_OP,
                  MessageConstants.MSG_OP_COURSE_CONTENT_REORDER)
              .addHeader(RouteConstants.ID_COURSE, courseId);
      eb.send(MessagebusEndpoints.MBEP_COURSE,
          new RouteRequestUtility().getBodyForMessage(routingContext),
          options,
    ...
    router.get(RouteConstants.EP_COURSE_LIST).handler(routingContext -> {
      DeliveryOptions options =
          DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
              .setSendTimeout(mbusTimeout * 1000)
              .addHeader(MessageConstants.MSG_HEADER_OP, MessageConstants.MSG_OP_COURSE_LIST_GET);
      eb.send(MessagebusEndpoints.MBEP_COURSE,
          new RouteRequestUtility().getBodyForMessage(routingContext),
          options,
          reply -> new RouteResponseUtility().responseHandler(routingContext, reply, LOGGER));
    ...
    router.get(RouteConstants.EP_COURSE_RESOURCES_GET).handler(routingContext -> {
      String courseId = routingContext.request().getParam(RouteConstants.ID_COURSE);
      DeliveryOptions options =
          DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
              .setSendTimeout(mbusTimeout * 1000)
              .addHeader(MessageConstants.MSG_HEADER_OP,
                  MessageConstants.MSG_OP_COURSE_RESOURCES_GET)
              .addHeader(RouteConstants.ID_COURSE, courseId);
      eb.send(MessagebusEndpoints.MBEP_COURSE,
          new RouteRequestUtility().getBodyForMessage(routingContext),
          options,
    ...
    router.get(RouteConstants.EP_COURSE_ASSESSMENTS_GET).handler(routingContext -> {
      ...
      DeliveryOptions options =
          DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
              .setSendTimeout(mbusTimeout * 1000)
              .addHeader(MessageConstants.MSG_HEADER_OP,
                  MessageConstants.MSG_OP_COURSE_ASSESSMENTS_GET)
              .addHeader(RouteConstants.ID_COURSE, courseId);
      eb.send(MessagebusEndpoints.MBEP_COURSE,
          new RouteRequestUtility().getBodyForMessage(routingContext),
          options,
    ...
    router.get(RouteConstants.EP_COURSE_COLLECTIONS_GET).handler(routingContext -> {
      String courseId = routingContext.request().getParam(RouteConstants.ID_COURSE);
      DeliveryOptions options =
          DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
              .setSendTimeout(mbusTimeout * 1000)
              .addHeader(MessageConstants.MSG_HEADER_OP,
                  MessageConstants.MSG_OP_COURSE_COLLECTIONS_GET)
              .addHeader(RouteConstants.ID_COURSE, courseId);
      eb.send(MessagebusEndpoints.MBEP_COURSE,
          new RouteRequestUtility().getBodyForMessage(routingContext),
          options,
