...
public class RouteAssessmentExternalConfigurator implements RouteConfigurator {
  ...
  private static final Logger LOGGER = LoggerFactory
      .getLogger(RouteAssessmentExternalConfigurator.class);
      ...
      DeliveryOptions options =
          DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
              ...
              .addHeader(MessageConstants.MSG_HEADER_OP,
                  MessageConstants.MSG_OP_EXT_ASSESSMENT_UPDATE)
              .addHeader(RouteConstants.ID_ASSESSMENT, assessmentId);
      eb.send(MessagebusEndpoints.MBEP_ASSESSMENT,
          new RouteRequestUtility().getBodyForMessage(routingContext),
          options,
    ...
    router.delete(RouteConstants.EP_ASSESSMENT_EX_DELETE).handler(routingContext -> {
      ...
      DeliveryOptions options =
          DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
              .setSendTimeout(mbusTimeout * 1000)
              .addHeader(MessageConstants.MSG_HEADER_OP,
                  MessageConstants.MSG_OP_EXT_ASSESSMENT_DELETE)
              .addHeader(RouteConstants.ID_ASSESSMENT, assessmentId);
      eb.send(MessagebusEndpoints.MBEP_ASSESSMENT,
          new RouteRequestUtility().getBodyForMessage(routingContext),
          options,
    ...
    router.post(RouteConstants.EP_ASSESSMENT_EX_CREATE).handler(routingContext -> {
      DeliveryOptions options =
          DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
              .setSendTimeout(mbusTimeout * 1000)
              .addHeader(MessageConstants.MSG_HEADER_OP,
                  MessageConstants.MSG_OP_EXT_ASSESSMENT_CREATE);
      eb.send(MessagebusEndpoints.MBEP_ASSESSMENT,
          new RouteRequestUtility().getBodyForMessage(routingContext),
          options,
          reply -> new RouteResponseUtility().responseHandler(routingContext, reply, LOGGER));
