...
class RouteLessonPlanConfigurator implements RouteConfigurator {

  private static final Logger LOGGER = LoggerFactory.getLogger(RouteLessonPlanConfigurator.class);
  ...
  public void configureRoutes(Vertx vertx, Router router, JsonObject config) {
    ...
    router.post(RouteConstants.EP_LESSON_PLAN_CREATE).handler(routingContext -> {
      ...
      DeliveryOptions options = DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
          ...
          .addHeader(MessageConstants.MSG_HEADER_OP, MessageConstants.MSG_OP_LESSON_PLAN_CREATE)
    ...
    router.put(RouteConstants.EP_LESSON_PLAN_UPDATE).handler(routingContext -> {
      String courseId = routingContext.request().getParam(RouteConstants.ID_COURSE);
      String unitId = routingContext.request().getParam(RouteConstants.ID_UNIT);
      String lessonId = routingContext.request().getParam(RouteConstants.ID_LESSON);
      String lessonPlanId = routingContext.request().getParam(RouteConstants.ID_LESSON_PLAN);
      DeliveryOptions options = DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
          .setSendTimeout(mbusTimeout * 1000)
          .addHeader(MessageConstants.MSG_HEADER_OP, MessageConstants.MSG_OP_LESSON_PLAN_UPDATE)
          .addHeader(RouteConstants.ID_COURSE, courseId).addHeader(RouteConstants.ID_UNIT, unitId)
          .addHeader(RouteConstants.ID_LESSON, lessonId)
          .addHeader(RouteConstants.ID_LESSON_PLAN, lessonPlanId);
      eb.send(MessagebusEndpoints.MBEP_COURSE,
          new RouteRequestUtility().getBodyForMessage(routingContext), options,
          reply -> new RouteResponseUtility().responseHandler(routingContext, reply, LOGGER));
    });

    router.delete(RouteConstants.EP_LESSON_PLAN_DELETE).handler(routingContext -> {
      String courseId = routingContext.request().getParam(RouteConstants.ID_COURSE);
      String unitId = routingContext.request().getParam(RouteConstants.ID_UNIT);
      String lessonId = routingContext.request().getParam(RouteConstants.ID_LESSON);
      String lessonPlanId = routingContext.request().getParam(RouteConstants.ID_LESSON_PLAN);
      ...
      DeliveryOptions options = DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
          .setSendTimeout(mbusTimeout * 1000)
          .addHeader(MessageConstants.MSG_HEADER_OP, MessageConstants.MSG_OP_LESSON_PLAN_DELETE)
          .addHeader(RouteConstants.ID_COURSE, courseId).addHeader(RouteConstants.ID_UNIT, unitId)
          .addHeader(RouteConstants.ID_LESSON, lessonId)
          .addHeader(RouteConstants.ID_LESSON_PLAN, lessonPlanId);
      eb.send(MessagebusEndpoints.MBEP_COURSE,
