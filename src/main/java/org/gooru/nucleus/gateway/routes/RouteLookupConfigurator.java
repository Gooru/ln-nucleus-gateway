      ...
      DeliveryOptions options =
          DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
              ...
              .addHeader(MessageConstants.MSG_HEADER_OP,
                  MessageConstants.MSG_OP_LKUP_ACCESS_HAZARDS);
      eb.send(MessagebusEndpoints.MBEP_LOOKUP,
          new RouteRequestUtility().getBodyForMessage(routingContext),
          options, reply -> new RouteResponseUtility().responseHandler(routingContext, reply, LOG));
    });
    ...
    router.get(RouteConstants.EP_MEDIA_FEATURES_LIST).handler(routingContext -> {
      DeliveryOptions options =
          DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
              .setSendTimeout(mbusTimeout * 1000)
              .addHeader(MessageConstants.MSG_HEADER_OP,
                  MessageConstants.MSG_OP_LKUP_MEDIA_FEATURES);
      eb.send(MessagebusEndpoints.MBEP_LOOKUP,
          new RouteRequestUtility().getBodyForMessage(routingContext),
          options, reply -> new RouteResponseUtility().responseHandler(routingContext, reply, LOG));
    });
    ...
    router.get(RouteConstants.EP_21_CENTURY_SKILLS_LIST).handler(routingContext -> {
      DeliveryOptions options =
          DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
              .setSendTimeout(mbusTimeout * 1000)
              .addHeader(MessageConstants.MSG_HEADER_OP,
                  MessageConstants.MSG_OP_LKUP_21_CEN_SKILLS);
      eb.send(MessagebusEndpoints.MBEP_LOOKUP,
          new RouteRequestUtility().getBodyForMessage(routingContext),
          options, reply -> new RouteResponseUtility().responseHandler(routingContext, reply, LOG));
    });
    ...
    router.get(RouteConstants.EP_SCHOOLDISTRICTS_LIST).handler(routingContext -> {
      DeliveryOptions options =
          DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
              .setSendTimeout(mbusTimeout * 1000)
              .addHeader(MessageConstants.MSG_HEADER_OP,
                  MessageConstants.MSG_OP_LKUP_SCHOOLDISTRICTS);
      eb.send(MessagebusEndpoints.MBEP_LOOKUP,
          new RouteRequestUtility().getBodyForMessage(routingContext),
          options, reply -> new RouteResponseUtility().responseHandler(routingContext, reply, LOG));
    });
    ...
    router.get(RouteConstants.EP_APIKEY_CONFIG).handler(routingContext -> {
      DeliveryOptions options =
          DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
              .setSendTimeout(mbusTimeout * 1000)
              .addHeader(MessageConstants.MSG_HEADER_OP,
                  MessageConstants.MSG_OP_LKUP_APIKEY_CONFIG);
      eb.send(MessagebusEndpoints.MBEP_LOOKUP,
          new RouteRequestUtility().getBodyForMessage(routingContext),
          options, reply -> new RouteResponseUtility().responseHandler(routingContext, reply, LOG));
    });
    ...
    router.get(RouteConstants.EP_JWT_GENERATION).handler(routingContext -> {
      DeliveryOptions options =
          DeliveryOptionsBuilder.buildWithApiVersion(routingContext)
              .setSendTimeout(mbusTimeout * 1000)
              .addHeader(MessageConstants.MSG_HEADER_OP,
                  MessageConstants.MSG_OP_FIREBASE_JWT_CREATION);
      eb.send(MessagebusEndpoints.MBEP_LOOKUP,
          new RouteRequestUtility().getBodyForMessage(routingContext),
          options, reply -> new RouteResponseUtility().responseHandler(routingContext, reply, LOG));
    });
