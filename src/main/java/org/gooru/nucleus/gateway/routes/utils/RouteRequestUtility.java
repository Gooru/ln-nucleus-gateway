...
public class RouteRequestUtility {
  ...
  public JsonObject getBodyForMessage(RoutingContext routingContext, JsonObject additionalBody) {
    ...
    result.put(MessageConstants.MSG_KEY_SESSION,
        (JsonObject) routingContext.get(MessageConstants.MSG_KEY_SESSION));
    result.put(MessageConstants.MSG_USER_ID,
        (String) routingContext.get(MessageConstants.MSG_USER_ID));
    result.put(MessageConstants.MSG_HEADER_TOKEN,
        (String) routingContext.get(MessageConstants.MSG_HEADER_TOKEN));
    return result;
  }
}